...
3. Ensure that you are *not* using a provisioning profile with a wildcard (`*`) in the bundle identifier (`com.chaione.*`). Team Provisioning Profiles auto-generated by Xcode will *not* work. Push notifications only work with full, explicit bundle identifiers (ex. `com.chaione.notifym`e) manually generated by memebers of your Apple Developer Program who have "Agent" status or higher. If you cannot make approve certificate signing requests on your own, talk to someone with "Agent" status or higher on your team to get them to approve them for you.
